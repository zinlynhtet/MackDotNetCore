...
public static class BlogService
{
    ...
    {
        app.MapGet("/blog", async ([FromServices] AppDbContext db, ILogger<Program> _logger) =>
        {
            var lst = await db.Blogs.AsNoTracking().ToListAsync();
            _logger.LogInformation("BlogLists =>" + JsonSerializer.Serialize(lst));
            return (lst);
        })
        .WithName("GetBlog")
        .WithOpenApi();
